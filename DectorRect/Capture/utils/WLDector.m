...
+ (UIImage *)dectorImage:(UIImage *)UIImg
{
    ...
    // 获取边缘识别最大矩形
    CIRectangleFeature *rectangleFeature = [WLDector biggestRectangleInRectangles:
                                            [[WLDector highAccuracyRectangleDetector] featuresInImage:CIImg]];

    if (rectangleFeature)
    {
        CIImg = [WLDector correctPerspectiveForImage:CIImg withFeatures:rectangleFeature];
    }
...
+ (CIImage *)correctPerspectiveForImage:(CIImage *)image withPointInfo:(NSArray *)pointArr
{
    ...
    if ([pointArr isKindOfClass:[NSArray class]] && 4 == pointArr.count) {
        ...
        CGPoint p0 = [WLDector convertCGPoint:[pointArr[0] CGPointValue] fromSize1:imageSize toSize2:screenSize];
        CGPoint p1 = [WLDector convertCGPoint:[pointArr[1] CGPointValue] fromSize1:imageSize toSize2:screenSize];
        CGPoint p2 = [WLDector convertCGPoint:[pointArr[2] CGPointValue] fromSize1:imageSize toSize2:screenSize];
        CGPoint p3 = [WLDector convertCGPoint:[pointArr[3] CGPointValue] fromSize1:imageSize toSize2:screenSize];
