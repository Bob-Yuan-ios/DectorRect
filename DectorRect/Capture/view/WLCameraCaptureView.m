...
@interface WLCameraCaptureView()<AVCaptureVideoDataOutputSampleBufferDelegate>
{
...
@end
@implementation WLCameraCaptureView
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error)
     {
         ...
         if (strongSelf.isBorderDetectionEnabled)
         {
             ...
             if (rectangleDetectionConfidenceHighEnough(self->_imageDedectionConfidence))
              {
                  // 获取边缘识别最大矩形
...
//             completionHandler([UIImage imageWithData:imageData], &df);
         ...
         if (self->_rectOverlay) {
             // 移除识别边框
             self->_rectOverlay.path = nil;
         }
          // 隐藏照相机视图
